...
/*   Updated: 2022/12/11 19:12:35 by emlicame         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int32_t	st_built_redirection_heredoc(t_token *tok, t_input *data)
{
	int	bites;
	int	fd_hd;
...
int32_t	built_open_infiles(t_token *tok, t_input *data)
{
	...
	while (tok && tok->type != PIPE)
	{
		if (tok->type == LESS)
		{
			...
			if (data->fds[READ] < 0)
				return (error_print(tok->next->str), 2);
		}
		else if (tok->type == DLESS)
		{
			ret = 1;
			if (st_built_redirection_heredoc(tok, data))
				return (error_print("heredoc file error"), 2);
		}
		tok = tok->next;
	}
	return (ret);
...
static int32_t	st_built_redirection_dgreat(t_token *tok, t_input *data)
{
...
int32_t	built_open_outfiles(t_token *tok, t_input *data)
{
	...
	while (tok && tok->type != PIPE)
	{
		...
		{
			ret = 1;
			if (st_built_redirection_dgreat(tok, data))
				return (error_print(tok->next->str), 2);
		}
		tok = tok->next;
	}
